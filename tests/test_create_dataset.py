...
from pygeochemtools.create_dataset import clean_dataset, convert_oxides, convert_ppm
...
def test_clean_dataset():
    ...
    result = clean_dataset(test_df).reset_index().drop(["index"], axis=1)
...
def test_convert_oxides():
    ...
    test_df = pd.read_csv(StringIO(test_df), sep="\t")
    expected_df = pd.read_csv(StringIO(expected_df), sep="\t", dtype={"VALUE": float})
    result = convert_oxides(test_df, element="Fe2O3")
...
def test_convert_ppm():
    ...
    test_df = """
    SAMPLE_NO	SAMPLE_SOURCE_CODE	DRILLHOLE_NUMBER	DH_DEPTH_FROM	DH_DEPTH_TO	SAMPLE_ANALYSIS_NO	ANALYSIS_TYPE_DESC	LABORATORY	CHEM_CODE	VALUE	UNIT	CHEM_METHOD_CODE	LONGITUDE_GDA2020	LATITUDE_GDA2020	BDL
    9661	DC	6363.0	56.69	57.91	5458	GEOCHEMISTRY	NaN	Fe2O3	15.0	ppm	NaN	134.541359	-30.703724	1
    ...
    9661	DC	6363.0	56.69	57.91	5458	GEOCHEMISTRY	NaN	Fe2O3	15.0	ppm	NaN	134.541359	-30.703724	1	0.001
    ...
    test_df = pd.read_csv(StringIO(test_df), sep="\t")
    expected_df = pd.read_csv(StringIO(expected_df), sep="\t", dtype={"VALUE": float})
    result = convert_ppm(test_df, element="Fe2O3")
